...
export default function SingleNoti({ noti, setIsOpen }) {
    ...
    return (
        <div onClick={() => {
            ...
            <img src={noti.senderPhotoURL || defaultProfile} alt="Profile" className='w-12 h-12 rounded-full' />
            {noti.isAnnouncement ? <div className="absolute flex items-center justify-center w-5 h-5 bg-yellow-500 rounded-full left-12 top-10">
                <TfiAnnouncement className="text-white size-3 " />
            </div> : noti.isComment ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`absolute ${isDark ? 'text-light' : 'text-blue-500'} size-5 left-12 top-9`}>
