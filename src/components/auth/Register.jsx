        ...
        <div className="w-full max-w-lg px-5 mx-auto mt-16 md:px-0">
            <form onSubmit={handleSubmit(signUpUser)} className={`px-8 pt-6 pb-8 mb-4 rounded  ${isDark ? 'bg-dbg shadow-custom-white' : 'bg-white drop-shadow-lg'}`}>
                ...
                <div className="mb-4">
                    ...
                    <input disabled={loading} {...register('username')} className={cn(errors.username && 'border-red-600 placeholder:text-red-500', "w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none disabled:cursor-not-allowed focus:outline-none focus:shadow-outline")} id="username" type="text" placeholder="Username" />
                    {errors.username &&
                ...
                <div className="mb-4">
                    ...
                    <input disabled={loading} {...register('email')} className={cn(errors.email && 'border-red-600 placeholder:text-red-500', "w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-non disabled:cursor-not-allowed focus:shadow-outline")} id="email" type="text" placeholder="Email" />
                    {errors.email &&
                        <div className="flex items-center gap-1 mt-1">
                            <span className="text-[16px] text-red-600 material-symbols-outlined">
                                error
                ...
                <div className="relative mb-4">
                    ...
                    <input disabled={loading} {...register('password')} className={cn(errors.password && 'border-red-600 placeholder:text-red-500', "w-full px-3 py-2 disabled:cursor-not-allowed leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline")} id="password" type={`${pVisible ? 'text' : 'password'}`} placeholder="Password" />
                ...
                <div className="mb-3">
                    ...
                    <div className="flex items-center justify-between">
                        <input disabled={loading} type="file" onChange={handleImageChange} name="" id="" className={`${isDark ? 'text-white' : ''}`} />
                        {!!pPreview && <img src={pPreview} className="rounded-full disabled:cursor-not-allowed h-7 w-7" alt="" />}
                    </div>
                </div>
                <div className="flex items-center justify-between mt-5">
                    <button disabled={loading} className="flex items-center gap-1 px-4 py-2 font-bold text-white transition duration-500 ease-in-out rounded disabled:cursor-not-allowed bg-primary hover:bg-blue-700 focus:outline-none focus:shadow-outline" type="submit">
